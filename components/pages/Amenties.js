  ...
  return (
    ...
    >
      <div className="flex flex-col items-start md:items-center gap-3 mb-4 md:mb-8">
        <h1 className="text-dark-500 font-serif ml-4 uppercase text-xl sm:text-2xl md:text-6xl tracking-[3.6px] sm:tracking-[6px] md:tracking-[9.6px] font-bold  ">
          Our Amenities
        </h1>
      ...
      <div className="flex flex-wrap justify-center sm:justify-start gap-4 sm:gap-6 md:gap-8">
        {amentiesItems.map((amenity, index) => (
          <div
            key={index}
            className="flex flex-col items-start text-left bg-dark-500 w-[90%] sm:w-2/3 md:w-[278px] h-64 sm:h-[268px] p-12 sm:px-10  sm:py-12 md:px-11 md:py-10 justify-center gap-4"
          >
            <Image
              src={amenity.icon}
              alt={amenity.title}
